<template>
    <div class="view">
        <div class="view-header">
            <h1 class="view-title"><i class="fa-solid fa-pen"></i> Edit Recurring Transaction</h1>
        </div>
        ...
        <form v-else class="rec-form" @submit.prevent="submit">
            ...
            <div class="form-actions">
                <FfButton variant="ghost" type="button" @click="router.push(`/recurring/${id}`)">Cancel</FfButton>
...
onMounted(async () => {
    ...
    form.transaction_type = tx.type ?? 'withdrawal';
    form.amount = tx.amount ?? '';
    form.source_id = String(tx.source_id ?? '');
    form.destination_id = String(tx.destination_id ?? '');
    form.repeat_freq = attr.repeat_freq ?? 'monthly';
    form.first_date = attr.first_date ?? new Date().toISOString().slice(0, 10);
    form.repetitions = attr.repetitions ?? 0;
...
async function submit() {
    ...
    try {
        ...
        toast.error(e.response?.data?.message ?? 'Failed to update recurring transaction.');
    } finally {
        saving.value = false;
    }
}
